...
const reviewSlice = createSlice({
  ...
  reducers: {
    getReviewByMovie: (
      state,
      action: PayloadAction<{ reviews: ReviewPayload; isLoadmore?: boolean }>,
    ) => {
    ...
    likeReviewSuccess: (state, action: PayloadAction<ReviewRepsonse>) => {
      ...
      const index = newReviews.reviewList.findIndex(
        (item: ReviewRepsonse) => item._id === action.payload._id,
      );
      if (index !== -1) {
